...
function updatePlayer(dt) {
  ...
  // ---- dragon transform
  // visYaw: the shared visual heading. Steering yaws BOTH the dragon root and
  // the camera by the same amount, so on screen the dragon never rotates — it
  // stays dead astern and banks while the whole view swings through the turn.
  Player.lean = damp(Player.lean || 0, ix, 4.2, dt);
  var visYaw = yaw + Player.lean * 0.40;
  dragon.root.position.copy(Player.pos);
  dragon.root.rotation.y = visYaw;
  ...
  // The camera sits dead astern of visYaw — the SAME heading the dragon root
  // uses — so steering rotates the world around a dragon that stays put on
  // screen, only banking. The camera's yaw lags the dragon's by a breath
  // (0.40 vs 0.34) so the turn still has parallax without exposing his flank.
  var lean = Player.lean;
  var camYawV = yaw + lean * 0.34;
  ...
  camera.lookAt(look);
  camera.rotateZ(-lean * 0.055);         // a breath of roll into the turn
